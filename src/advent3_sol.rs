...
pub fn part_two(input: &str) -> u32 {
    ...
    let oxy_rating =
    find_line_by_bit_criteria(|a, b| { println!("a: {}",a.len()); if a.len() >= b.len() { a } else { b }}, &lines);

    let co2_rating =
    find_line_by_bit_criteria(|a, b| { println!("b: {}", b.len()); if a.len() >= b.len() { b } else { a }}, &lines);

    println!("{}", str_to_int(oxy_rating) * str_to_int(co2_rating));
